...
export class VirtuaGymClientV1 {
  ...
  /** Retrieves every credit row matching the query across all pages. */
  public async allMemberCredits(
    options: MemberCreditsOptions = {},
  ): Promise<MemberCredit[]> {
    const credits: MemberCredit[] = [];
    for await (const page of this.memberCredits(options)) {
      credits.push(...page);
    }
    return credits;
  }
  ...
  /**
   * Yields credit rows page by page, fetching each page lazily. Each row is
   * the credits of one member in one service type. Without memberId the
   * whole club is listed.
   */
  public async *memberCredits(
    options: MemberCreditsOptions = {},
    ...
    for (;;) {
      ...
      const { result, status } = await this.request(
        z.array(memberCreditSchema),
        {
          method: 'get',
          path: `club/${this.options.clubId}/credit`,
          contentType: 'application/json',
          params,
        },
      );
      ...
      if (next?.syncFrom !== undefined && next.syncFrom !== syncFrom) {
        syncFrom = next.syncFrom;
      } else if (last.timestamp_edited > syncFrom) {
        syncFrom = last.timestamp_edited;
      } else {
        return;
      }
    }
  ...
  /**
   * Assigns credits to a member for one service type. One member and one
   * service type per request.
   *
   * The client_id GUID deduplicates: replaying the same client_id succeeds
   * with the message "Already picked up or completed" instead of allocating
   * twice.
   */
  public async addMemberCredits(
    data: AddMemberCreditsData,
  ): Promise<CreditTransaction> {
    const { result } = await this.request(creditTransactionSchema, {
      method: 'put',
      path: `club/${this.options.clubId}/credit`,
      contentType: 'application/json',
      data,
...
export interface MemberCreditsOptions {
  /**
   * Only credit rows edited on/after this timestamp — in SECONDS live,
   * although the docs claim milliseconds. Defaults to 0.
   */
  readonly syncFrom?: number;
  /** Only credits of this member. */
  readonly memberId?: number;
}

/**
 * Body for assigning credits. Exactly one of member_id/member_email and
 * exactly one of credit_amount/credit_unlimited must be given. Names match
 * the API's wire format.
 */
export type AddMemberCreditsData = {
  /**
   * Normalized service type (lowercase, spaces as hyphens), e.g. "access",
   * "test-credit".
   */
  readonly service_type: string;
  /** Date (Y-m-d) until when the credits are valid; omit for unlimited validity. */
  readonly valid_until?: string;
  /** Why the credits are added; truncated to 200 chars by the API. */
  readonly notes?: string;
  /**
   * 36-char GUID generated by you. The API deduplicates on it, so retrying
   * the same request is safe.
   */
  readonly client_id: string;
} & (
  | { readonly member_id: number; readonly member_email?: never }
  | { readonly member_email: string; readonly member_id?: never }
) &
  (
